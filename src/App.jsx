...
function getDutchDateTime() {
  const now = new Date();
  return {
    datum: now.toLocaleDateString("nl-NL", { weekday:"long", day:"numeric", month:"long" }),
...
const ADKHAR = {
  ...
  avond: [
    ...
    { id:"a16", arabic:"يَا حَيُّ يَا قَيُّومُ بِرَحْمَتِكَك أَسْتَغِيثُ", fonetisch:"Yaa Hayyu yaa Qayyoomu birahmatika astagheethu...", vertaling:"O Eeuwig Levende, o Zelfbestaande — door Uw genade zoek ik hulp...", aantal:1, categorie:"Dhikr", beloning:null },
...
export default function AdkharApp() {
  ...
  if (!geladen) return (
    <div style={{ minHeight:"100vh", background:"var(--cream)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ fontFamily:"'Cormorant Garamond', Georgia, serif", fontSize:"0.9rem", color:"var(--ink-faint)", letterSpacing:"0.2em" }}>laden…</div>
    </div>
  );
      ...
      <div className="paper" style={{ minHeight:"100vh", background:"var(--cream)", fontFamily:"'Cormorant Garamond', Georgia, serif", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"32px 0 0" }}>
          {sessions.map((s, i) => {
            ...
            return (
              <div key={s.type} onClick={() => startSessie(s.type)}
                style={{ borderTop: i===0 ? "1px solid var(--cream-deep)" : "none", borderBottom:"1px solid var(--cream-deep)",
                  padding:"22px 28px", cursor:"pointer", display:"flex", alignItems:"center", gap:20,
                  transition:"background 0.2s", animation:`fadeUp 0.55s ${0.15+i*0.1}s ease both`, background:"transparent" }}
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--cream)", display:"flex", flexDirection:"column", fontFamily:"'Cormorant Garamond', Georgia, serif", userSelect:"none" }}>
      ...
      <div style={{ height:2, background:"var(--cream-deep)", margin:"14px 24px 0", borderRadius:1 }}>
        <div style={{ height:"100%", width:`${(voltAantal/adkharLijst.length)*100}%`,
          background:`linear-gradient(90deg, ${c.accent}88, ${c.accent})`, borderRadius:1, transition:"width 0.5s ease" }}/>
      </div>
      ...
      {klaar ? (
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"0 36px", textAlign:"center" }}>
          <div className="complete-pop" style={{ width:76, height:76, borderRadius:"50%", background:"linear-gradient(135deg, #D4EAD9, #B8E0C2)",
            display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.8rem", marginBottom:28, boxShadow:"0 8px 32px rgba(74,124,89,0.2)" }}>
            ✓
          </div>
          ...
          <div style={{ fontSize:"1.05rem", color:"var(--ink-soft)", marginBottom:8 }}>Alle {sessie}adhkaar voltooid</div>
          <div style={{ fontSize:"0.95rem", color:"var(--ink-muted)", fontStyle:"italic", marginBottom:52 }}>Moge Allah het van jou aanvaarden</div>
          <button onClick={() => setScherm("home")}
            style={{ padding:"14px 40px", background:"var(--ink)", border:"none", borderRadius:2, color:"var(--cream)",
              fontFamily:"'Cormorant SC', serif", fontSize:"0.78rem", letterSpacing:"0.18em", textTransform:"uppercase", cursor:"pointer" }}>
            Terug naar home
          </button>
        </div>
      ) : (
        <>
          ...
          {/* Tikzone */}
          <div onClick={handleTik}
            style={{ position:"relative", margin:"0 20px 20px", borderRadius:4,
              ...
              boxShadow:"0 2px 16px rgba(0,0,0,0.05)", WebkitTapHighlightColor:"transparent" }}
          >
            {ripples.map(r => (
              <div key={r.id} style={{ position:"absolute", left:r.x, top:r.y, width:12, height:12,
                background:c.accent+"25", borderRadius:"50%", transform:"translate(-50%,-50%) scale(0)",
